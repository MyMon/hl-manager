(function($){
	...
	for (var i = 0; i < playlist.length; i++){
		var item = playlist[i];
		$('#playlist').prepend('<li class="lib" style="overflow:hidden;"><strong style="margin-left: 5px;">'+item.title+'</strong><span style="float: right;" class="artist">'+item.artist+'</span></li>');
		if (item.mp3 == "") {
	...
	if (isShuffle && shuffle_array != undefined && playlist.length === (shuffleArray=JSON.parse(shuffle_array)).length) {
		...
		shuffleIndex = 0;
	    $('#QPlayer .cover').attr('title', '1');
	} else {
		isShuffle = false;
	    $('#QPlayer .cover').attr('title', '2');
	}
	...
	$("#QPlayer .ssBtn").on('click', function(){
		...
		if ($('.ssBtn .adf').hasClass('on') === false) {
			if (isFirstPlay) {
			    setTimeout("showTips('#player .cover','添加歌曲在歌曲列表底部哦', " + function(){setTimeout("showTips('#player .ctrl .musicTag','爷知道了')", 1000)} + ");", 500);
	...
	$("#player .cover").on('click',function(){
		isShuffle = !isShuffle;
		if (isShuffle) {
	        $("#player .cover").attr("title","哪呢");
	        showNotification('what');
...
function showTips(div, info, func) {
	var box_height = 100;
	$('body').append('<div class="qplayer_tips"><span class="tips_arrow"></span><span class="info" style="display:none">' + info + '</span><button class="tips_button" onclick="removeTips()">what are you 干啥</button></div>');
